...
class VN30:
    ...
    def display_stock_status(self, df):
        ...
        df = df.reset_index()  # Reset index to use Datetime as a column
        grouped = df.groupby('StockSymbol').last()  # Get the last entry for each stock
        grouped['Color'] = grouped['Crash Risk'].map(color_map)
        
        stocks = grouped.index.tolist()
        crash_risks = grouped['Crash Risk'].tolist()
        colors = grouped['Color'].tolist()
        
        st.markdown("<div style='display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px;'>", unsafe_allow_html=True)
        for stock, crash_risk, color in zip(stocks, crash_risks, colors):
            st.markdown(f"""
                <div style='border: 2px solid #ddd; padding: 10px; border-radius: 5px; background-color: {color};'>
                    <strong>{stock}</strong><br>{crash_risk}
                </div>
            """, unsafe_allow_html=True)
