...
with st.sidebar.expander("Danh mục đầu tư", expanded=True):
    ...
    if portfolio_options:
        ...
        if not df_full.empty:
            ...
            selected_stocks.extend(selected_symbols_in_sector)
...
# Load the selected sector data if no portfolio is selected
...
if not df_full.empty and (start_date < first_available_date.date() or end_date > datetime.today().date()):
    st.error("Khoảng thời gian bạn chọn nằm ngoài phạm vi dữ liệu sẵn có.")
else:
