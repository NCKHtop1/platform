...
if not df_full.empty:
    ...
    first_available_date = datetime(2000, 1, 1)
    last_available_date = datetime.today()

start_date = st.date_input('Ngày bắt đầu', first_available_date.date())
end_date = st.date_input('Ngày kết thúc', last_available_date.date())

# Ensure that the date range is within the available data
if start_date < first_available_date.date() or end_date > last_available_date.date():
    st.warning(f"Khoảng thời gian bạn chọn nằm ngoài phạm vi dữ liệu sẵn có ({first_available_date.date()} - {last_available_date.date()}).")
